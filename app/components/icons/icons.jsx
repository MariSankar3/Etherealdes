...
export function Indicator({ className = "" }) {
    return <div className={`w-[15px] h-fit border-b ${className}`}></div>
}
...
export function Redirect({ className = "" }) {
    return <svg className={`w-8 h-8 text-[#DBF900] ${className}`}>
        <use href="/icons/icons.svg#redirect" />
    </svg>
}

export function Arrow({ className = "" }) {
    return <svg className={`text-[#FFFFFF] ${className}`}>
