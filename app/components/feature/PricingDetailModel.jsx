...
export default function PricingDetailModel({ open, onClose, pricingKey, cardRect, cardIndex }) {
  ...
  let style = {};
  let modalClass = "relative w-full max-w-[320px] bg-[#181818] rounded-2xl md:rounded-3xl border-2 border-[#4F4E4E] px-4 py-4 sm:p-6 flex flex-col items-center";
  ...
  if (isLg && cardRect) {
    ...
    const modalWidth = 320; // matches sm:max-w-[400px]
    const viewportWidth = window.innerWidth;
    let left = cardRect.left + window.scrollX + cardRect.width / 2 - modalWidth / 2;
    if (left < 16) left = 16; // px-4
    ...
    modalClass = "relative w-full max-w-[350px] bg-[#181818] rounded-3xl border border-[#4F4E4E] px-4 py-4 sm:p-6 flex flex-col items-center shadow-2xl";
  }
  if (isOverlay) {
    ...
    if (isLg) {
      ...
      modalClass = modalClass.replace(' h-full', '').replace('flex justify-center items-center', '');
          ...
          >
            ...
            <div className="w-full flex items-center justify-between pb-2 border-b border-[#4F4E4E]">
              <div className="flex flex-col items-start font-antonio">
                <h2 className="text-white text-2xl font-bold leading-tight mb-1">{data.title}</h2>
                <span className="text-[#DBF900] text-xl font-bold">{data.price}
                <span className="text-white text-sm font-light ml-1">/month</span> </span>
              </div>
              <Link href="/get-in-touch" className="text-black text-xs md:text-sm font-light bg-[#DBF900] px-2 md:px-4 py-2 rounded-md font-antonio tracking-[0.02em]" onClick={onClose}>
                Contact Us Now!
              </Link>
            </div>
            {/* Image */}
            <div className="flex justify-center items-center py-2">
              <img src={data.featureImage} alt={data.title} className="w-48 object-contain" />
            </div>
            {/* Features */}
                  ...
                  <div key={idx}>
                    <li className="text-white text-base font-antonio flex items-center tracking-[0.08em]">
                      {feature.split('/').map((word, index) => (
                        <span key={index} className={`${index > 0 ? 'text-[#ffffff7a] text-xs font-light ml-2' : 'font-[600]'}`}>
                          {index === 0 ? word : `/${word}`}
                        </span>
                      ))}
                    </li>
                    <div className={`border-b border-[#4F4E4E5a] my-1 w-full ${idx === data.features.length - 1 ? 'hidden' : ''}`}></div>
                  </div>
                ))}
              </ul>
            </section>
